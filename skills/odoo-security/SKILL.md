...
name: odoo-security
description: >-
  Security audit of Odoo addon code: access control (ir.model.access, ir.rule, field groups,
...
<!-- adapted for Odoo 19.0: ir.access is a 20.0 concept; use ir.model.access + ir.rule -->
Mechanics of `ir.model.access` rows, `ir.rule` record rules, and field `groups` are in the `odoo-guidelines`
...
<!-- adapted for Odoo 19.0: ACL columns are perm_read/perm_write/perm_create/perm_unlink in ir.model.access.csv; record rules (ir.rule) enforce domain restrictions -->
- Flag any `perm_write`/`perm_create`/`perm_unlink` granted to `base.group_portal` or
  `base.group_public`; scrutinize even `perm_read` on models holding personal
  or business-sensitive data. A row with an empty `group_id` applies to all users.
- Use `ir.rule` (record rules) in `security/<module>_security.xml` for domain-level
  restrictions. A global rule (no `groups`) is AND-ed onto every user; a group rule
  applies only to group members. Cover all CRUD operations that need restricting.
- Multi-company models need a global `ir.rule` with `[('company_id', 'in', company_ids)]`;
  sensitive models need domain restrictions, not just group permissions.
